...
| 状态管理 | WebAgentClient (单一数据源) |
| 事件系统 | EventManager (单向通知) |

### 全局接口设计

**架构目标**: 接口化、自治化，为 AI Agent 自主开发奠定基础

```javascript
// 统一命名空间（避免污染）
window.FreeWebAI = {
    // 核心接口
    WebAgentClient,  // 业务编排器（唯一入口）
    AIAgent,         // 基础设施（被管理）
    EventManager,    // 事件总线（单向通知）
    
    // 便捷方法
    getState() {     // 查询完整状态
        return WebAgentClient.getState();
    },
...
**设计原则**:
- ✅ **单一入口** - 所有操作通过 WebAgentClient
- ✅ **接口稳定** - 对外暴露的 API 保持向后兼容
- ✅ **自治能力** - AI 可通过接口自我完善和扩展
...
{
    init(options),                    // 初始化
    handleUserMessage(message, opts), // 处理用户消息（直接调用）
    handleCodeExecution(code, opts),  // 执行代码（直接调用）
    extractAndExecuteCode(text),      // 自动提取并执行代码
    handleClearChat(),                // 清空对话（直接调用）
...
**设计原则**:
- ✅ UI 层通过**直接调用**与 WebAgentClient 交互
- ✅ WebAgentClient 通过**事件通知** UI 层状态变化
- ✅ 所有错误统一通过 `handleError()` 报告和追踪
...
[UI Layer] useAgent.sendMessage()
  ├─ **直接调用** WebAgentClient.handleUserMessage(message)
  └─ 创建 UI 占位符消息
  ↓
...
[Business Layer] WebAgentClient.onChunk(chunk)
  └─ **事件通知**: MESSAGE_STREAMING { chunk }
  ↓
[UI Layer] useAgent 监听事件
  └─ 更新 React 状态 → 显示流式内容
  ↓
...
**关键改进**:
- ✅ UI → Client: **直接调用** (同步、类型安全、易调试)
- ✅ Client → UI: **事件通知** (解耦、异步、可扩展)
