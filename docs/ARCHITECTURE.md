...
项目采用 **MVC 分层架构**，各层通过**事件总线**和**接口规范**进行通信，实现高度解耦。
...
│  StoragePage                                                 │
│  - 负责 DOM 渲染与用户交互                                    │
│  - 每个 Page 对应一个 EventHandler 处理事件                   │
└─────────────────────────────────────────────────────────────┘
                              │ EventBus
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                 Controller Layer                             │
│  ChatController | SessionController | SettingsController    │
│  StorageController | ScriptsController                      │
│  - 业务逻辑协调                                              │
│  - 委托给 Service 层处理具体操作                              │
└─────────────────────────────────────────────────────────────┘
                              │ Facade Pattern
                              ▼
┌─────────────────────────────────────────────────────────────┐
│              Service Center (服务管理中心)                    │
│  ServiceCenter | ServiceRegistry                            │
│  - 统一管理服务实例的生命周期                                  │
│  - Facade 模式组装 API 能力与 UI 交互逻辑                     │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│                   Core Layer                                 │
│  EventBus | Events                                          │
│  Models: Message, Session, Settings, Model, etc.            │
│  Stores: SessionManager, ToolRegistry                       │
└─────────────────────────────────────────────────────────────┘
```
