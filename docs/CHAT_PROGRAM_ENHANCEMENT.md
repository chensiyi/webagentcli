# ChatProgram 消息发送编排增强方案

> 基于现有架构（Microkernel v0.6.6）的渐进式改进，不破坏现有代码结构
...
用户输入
  │
  ▼ ChatPage.svelte → USER_APPLY_SEND
  ▼ ChatEventHandler → chat:cmd:send
  ▼ ChatProgram.sendMessage()
      ├── Provider 检查
      ├── 会话管理（创建/获取/更新）
      ├── 用户消息写入 Session + 自动标题
      ...
      └── 收到 tool_calls → ToolExecutor.execute()
              ├── 参数类型校验（ToolsManager._validateArgs）
              ├── 最多重试 3 次（ToolExecutor._invokeWithRetry）
              └── 工具结果写入 → 继续 sendMessage(isToolContinuation)
```
...
新增：
  ...
  - 删除进行中会话时自动取消（ChatProgram 订阅 SESSION_DELETED）
```
...
**文件**：`kernel/programs/chat/ContextBuilder.ts`
...
**文件**：`kernel/programs/chat/ToolExecutor.ts`
...
```
HistoryPage.svelte → SessionManager.deleteSession()
    │ 发射 SESSION_DELETED { sessionId }
    ▼
ChatProgram 订阅 SESSION_DELETED
    │ 匹配 _currentRequest.sessionId
    ▼
自动 cancel()
```
...
2. `kernel/programs/ChatProgram.ts` — 构造函数订阅，匹配则 cancel
...
#### 实现位置：ContextBuilder._buildSystemPrompt()
