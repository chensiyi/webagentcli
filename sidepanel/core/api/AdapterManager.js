...
class AdapterManager {
  ...
  /**
   * 拉取模型列表（使用当前适配器）
   * @param {Object} config - 配置对象（可选，否则使用适配器当前配置）
  ...
  async fetchModels(config) {
    ...
    if (typeof adapter.listModels !== 'function') {
      throw new Error(`Adapter ${this.currentAdapterName} does not support listModels`);
    }
    
    return await adapter.listModels(config || this.currentAdapter.config);
  }
  ...
  async detectCapabilities(modelName) {
    ...
    return await adapter.detectCapabilities(modelName, adapter.config);
  }
}
