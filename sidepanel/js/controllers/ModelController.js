/**
 * ModelController - 模型列表管理与能力检测控制器
 * 
 * 职责：
...
class ModelController {
  ...
  async fetchModels({ apiStandard, apiEndpoint, apiKey, forceRefresh = false }) {
    console.log('[ModelController] Fetching models:', { apiStandard, apiEndpoint, forceRefresh });

    const storage = this.serviceCenter.getStorageController();
    ...
    if (!forceRefresh) {
      ...
      if (cached && Array.isArray(cached)) {
        console.log('[ModelController] Using cached models for', apiEndpoint);
    ...
    console.log('[ModelController] Fetched and standardized', this.models.length, 'models');
    return this.models;
  }
  ...
  async clearCache(apiEndpoint) {
    ...
    console.log('[ModelController] Cache cleared for', apiEndpoint);
  }
