...
class ChatEventHandler {
  ...
  destroy() {
    this._listening = false;
    if (this.chatChannel) {
      this.chatChannel.off(Events.CHAT.USER_APPLY_SEND);
      this.chatChannel.off(Events.CHAT.USER_APPLY_STOP);
      this.chatChannel.off(Events.CHAT.USER_APPLY_DELETE_MESSAGE);
      this.chatChannel.off(Events.CHAT.MESSAGE_UPDATED);
      this.chatChannel.off(Events.CHAT.STREAM_CHUNK_APPEND);
      this.chatChannel.off(Events.CHAT.STREAM_COMPLETE);
      this.chatChannel.off(Events.CHAT.STREAM_ERROR);
      this.chatChannel.off(Events.CHAT.STREAM_START);
      this.chatChannel.off(Events.TOOL.EXECUTING);
      this.chatChannel.off(Events.TOOL.COMPLETED);
      this.chatChannel.off(Events.TOOL.ALL_COMPLETED);
    }
  }
  ...
  _registerEventListeners() {
    ...
    this.chatChannel.on(Events.CHAT.USER_APPLY_SEND, (data) => this._handleApplySend(data));
    this.chatChannel.on(Events.CHAT.USER_APPLY_STOP, () => this._handleApplyStop());
    this.chatChannel.on(Events.CHAT.USER_APPLY_DELETE_MESSAGE, (data) => this._handleApplyDeleteMessage(data));
  ...
  _handleMessageUpdated(data) {
    const { message } = data;
    if (!message) return;
    this._updateMessageContent(message.id, extractText ? extractText(message.content) : message.content);
    if (message.reasoning_content) {
      this._updateMessageReasoning(message.id, extractText ? extractText(message.reasoning_content) : message.reasoning_content);
    }
  }

  _handleStreamChunkAppend(data) {
    const { messageId, content, reasoning_content } = data;
    if (reasoning_content) this._handleStreamReasoning({ messageId, reasoning_content });
    if (content) this._updateMessageContent(messageId, content, true);
  }
